...
export const WIDGET_DOCS: WidgetDoc[] = [
...

  {
    ...
    name: "Rows Slicer",
    icon: "✂️",
    svgIcon: "regular_filter.svg",
    tagline:
      "Extract exactly the rows you want using start and end markers — built for exports where your data sits between known text labels.",
    description:
      "Targets a pattern common in real-world exports: a flat table that mixes data rows with section headers, totals, and labels. Instead of manually identifying row numbers, you define start and end text markers. The widget finds every matching pair and extracts the rows between them, discarding everything else.",
    useCases: [
      "Exports with repeating data blocks separated by section headers",
      "Accounting reports where each client's data starts with their name and ends with a Total row",
      "Government or system reports where multiple entities are stacked into one flat table",
      "Any table with interspersed header or footer rows that need to be stripped out",
    ],
    steps: [
      {
        title: "Connect a raw table",
        detail:
          "Connect the output of PDF Converter or a file import to Rows Slicer. The table typically contains a mix of data rows and label rows.",
      },
      {
        title: "Identify your marker values",
        detail:
          "Look at the raw data to find the text values that consistently mark the start and end of the section you want. For example, a start marker might be 'Revenue' and the end marker 'Total Revenue'.",
      },
      {
        title: "Select the target column",
        detail:
          "Choose which column the widget should scan for marker values. Usually this is the first column, but it can be any text column in the table.",
      },
      {
        title: "Define a marker pair",
        detail:
          "Click 'Add Pair'. Enter the start marker and the end marker. The widget will find the first row matching the start marker, then capture all rows until it hits the end marker.",
      },
      {
        title: "Add more pairs if needed",
        detail:
          "If you need rows from multiple sections, add more marker pairs. Each pair is evaluated independently and all results are combined in the output.",
      },
      {
        title: "Validate with the live row count",
        detail:
          "The footer shows how many rows were captured. If it shows zero, the marker values may not match exactly — check for extra spaces, capitalization differences, or invisible characters in the data.",
      },
    ],
    inputType: "Data Table",
    inputNotes: [
      "The table should contain at least one text column that holds the marker values",
      "Marker matching is exact and case-sensitive by default",
      "Works best when marker values appear consistently in the same column across all sections",
    ],
    outputType: "Data Table",
    outputNotes: [
      "Same columns as input — only rows between each marker pair are kept",
      "Multiple pairs produce a combined output — all matching sections are stacked into one table",
      "The marker rows themselves are not included in the output",
      "Row order within each extracted section is preserved",
    ],
    tips: [
      {
        type: "info",
        title: "Repeated sections are captured automatically",
        body: "If the same marker pair appears multiple times in the table — for example, 12 monthly sections all using the same labels — every occurrence is captured and stacked into the output.",
      },
      {
        type: "info",
        title: "Run Header Promoter first",
        body: "If your raw table has real column headers sitting in the first data row, run Header Promoter before Rows Slicer. This makes marker values more predictable and consistent.",
      },
      {
        type: "warning",
        title: "Matching is exact",
        body: "Extra spaces or inconsistent capitalization will cause markers to be missed. If you get zero rows, open the raw table and inspect the exact text in the marker column.",
      },
      {
        type: "warning",
        title: "Marker rows are not included in the output",
        body: "The start and end marker rows themselves are excluded. If a marker row also contains data you need, you will need to capture it separately.",
      },
    ],
  },
